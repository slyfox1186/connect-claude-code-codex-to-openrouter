...
class Boundaries(unittest.TestCase):
    ...
    def test_config_collection_shapes(self):
        for key, value in [
            ...
            ("roles", []),
            ("categories", ["coding"]),
            ("categories", {"coding": None}),
            ("allowed_models", "test/model"),
            ("deny_file_patterns", [4]),
            ("aliases", {"x": 7}),
            ("roles", {"advisor": []}),
            ("default_panel", 4),
        ]:
    ...
    def test_boolean_cannot_disable_numeric_guard(self):
        core._config_cache = dict(
            core.load_config(), max_cost_usd_per_call=False, max_input_chars=False
        )
    ...
    def test_malformed_error_fields_are_actionable(self):
        for error in [
            {"message": 4},
            {"message": "failed", "metadata": {"error_type": []}},
            {"code": "unavailable", "message": "failed"},
            "bad",
        ]:
    ...
    def test_http_error_body_is_bounded_and_closed(self):
        ...
        with (
            patch.object(core, "get_api_key", return_value="fake"),
    ...
    def test_broken_stream_is_not_retried_or_exposed_as_traceback(self):
        with (
            patch.object(core, "get_api_key", return_value="fake"),
            patch.object(
                core.urllib.request, "urlopen", side_effect=ConnectionResetError("reset")
            ) as request,
            self.assertRaises(core.OpenRouterError),
        ):
            core._request("POST", "/chat/completions", {})
    ...
    def test_verify_does_not_claim_offline_catalogue_is_current(self):
        with (
            patch.object(core, "get_catalog", side_effect=core.OpenRouterError("offline")),
            self.assertRaises(core.OpenRouterError),
        ):
    ...
    def test_thread_attachment_budget_is_total_across_turns(self):
        ...
        with patch.object(core, "THREAD_DIR", Path(SCRATCH.name) / "budget-thread"):
            self.assertTrue(
                core.save_thread("budget", "first", "a", "test/model", attachments=[part])
            )
            self.assertFalse(
                core.save_thread("budget", "second", "b", "test/model", attachments=[part])
            )
    ...
    def test_malformed_thread_parts_are_not_replayed(self):
        bad = [
            {"role": ["user"], "content": "x"},
            {
                ...
                "content": [
                    {"type": "file", "file": 42},
                    {"type": "input_audio", "input_audio": "bad"},
                ],
            },
        ]
    ...
    def test_annotation_content_counts_for_replay_and_storage(self):
        image = {
            "type": "image_url",
            "image_url": {"url": "data:image/png;base64," + base64.b64encode(b"X" * 100).decode()},
        }
        annotation = {
            "type": "file",
            "file": {
                "hash": "fixture",
                "content": [{"type": "text", "text": "parsed text" * 20}, image],
            },
        }
        ...
        with patch.object(core, "THREAD_DIR", Path(SCRATCH.name) / "annotation-thread"):
            self.assertFalse(
                core.save_thread("budget", "q", "a", "test/model", annotations=[annotation])
            )
    ...
    def test_oversized_transcript_does_not_replace_readable_history(self):
        with (
            patch.object(core, "THREAD_DIR", Path(SCRATCH.name) / "size-thread"),
