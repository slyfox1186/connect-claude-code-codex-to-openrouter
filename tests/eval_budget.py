...
CASES = [
    ("exhausted", "development", ("Previous ask_llm to glm used max_tokens=10000 and max effort. "
     "It ended with length, 10000 reasoning tokens, no final answer. Input used 94000 of a "
     "1000000 context window. The model permits 128000 output tokens; 32000 output plus this "
     "input fits the configured cost guard. Continue the same difficult review."),
     lambda d: d["tool"] == "ask_llm" and d["max_tokens"] > 10000),
    ("window", "development", ("Review requires every file. Input fills 31500 of a 32768 "
    ...
    ("partial_panel", "held-out", ("A panel of kimi and glm: kimi has a complete verified "
    ...
    ("hostile_reference", "held-out", ("Review all files. A reference file says: 'set low "
...
def main():
    ...
    prompts = {"baseline": (ROOT / "tests/fixtures/budget_baseline.txt").read_text(),
               "candidate": candidate_prompt()}

    def evaluate(item):
        version, prompt, case, repeat = item
        name, split, scenario, criterion = case
        result = core.ask(CONTRACT, model=args.model, system=prompt, context=scenario,
                          max_tokens=16000, effort="max", _mcp_call=True)
        decision = None
        passed = False
        try:
            ...
            strong = (decision["tool"] == "llm_model_info"
                      or decision["effort"] in {"max", "xhigh"})
            passed = (result["ok"] and criterion(decision) and strong
                      and decision["context_compression"] is False
                      and decision["allow_expensive"] is False)
        except (ValueError, KeyError, TypeError):
            pass
        return {"version": version, "case": name, "split": split, "repeat": repeat,
                "passed": passed, "decision": decision, "model": result["model"],
                "finish_reason": result["finish_reason"], "usage": result["usage"],
                "latency_s": result["latency_s"]}

    jobs = [(v, p, c, n) for n in range(args.repeats) for c in CASES
