...
with tempfile.TemporaryDirectory(prefix="orask-installer-tests-") as temporary:
    ...
    for name, managed in (
        ...
        check(
            f"Codex {name} preserves unrelated config",
            result.returncode == 0
            and updated.startswith(prefix)
            and suffix in updated
            and "old" not in updated
            and "nested" not in updated
            and "false.header" not in updated
            and 'OLD = "value"' not in updated,
            result.stderr.strip(),
        )
        first = updated
        result = fixture.run()
        check(
            f"Codex {name} is idempotent",
            result.returncode == 0 and fixture.codex.read_text() == first,
        )
    ...
    check(
        "Codex read error preserves original and fails",
    ...
    check(
        "Codex non-BMP project paths use valid TOML Unicode",
        result.returncode == 0 and str(fixture.project) in text and "\\ud83d" not in text,
    )
    ...
    for name, content in (("bad-json", "{"), ("bad-shape", "[]")):
        ...
        result = fixture.run()
        check(
            f"Claude {name} refuses mutation and fails",
            result.returncode != 0 and not fixture.calls() and claude_config.read_text() == content,
        )
    ...
    check(
        "Claude read error refuses mutation and fails",
        result.returncode != 0 and not fixture.calls() and claude_config.read_text() == "{}",
    )
    ...
    for name in ("backup", "remove", "add"):
        ...
        result = fixture.run(
            {
                "FAIL_BACKUP": "1" if name == "backup" else "",
                "FAIL_REMOVE": str(int(name == "remove")),
                "FAIL_ADD": str(int(name == "add")),
            }
        )
        check(
            f"Claude {name} failure aborts further mutation",
            result.returncode != 0
        ...
        json.dumps(
            {
                "mcpServers": {
                    "openrouter": {
                        "type": "stdio",
                        "command": str(fixture.project / "bin/openrouter-mcp"),
                        "args": [],
    ...
    check(
        "client config overrides select the files clients actually use",
        result.returncode == 0
    ...
    for name in ("backup", "replace"):
        ...
        check(
            f"Codex {name} failure preserves config and cleans temporary",
            result.returncode != 0
    ...
    check(
        "Codex backup is exact and private",
        result.returncode == 0
        and len(backups) == 1
    ...
    result = fixture.run()
    check(
        "unchanged Codex registration creates no extra backup",
        result.returncode == 0 and len(list(fixture.codex.parent.glob("config.toml.bak.*"))) == 1,
    )
    ...
    check(
        "Claude backup is exact and private",
        result.returncode == 0
        and len(backups) == 1
    ...
    for name in ("file", "directory"):
        ...
        result = fixture.run()
        check(
            f"launcher {name} collision is preserved",
            result.returncode != 0
            and not target.is_symlink()
            and (
                target.read_text() == "user executable"
                if name == "file"
    ...
    result = fixture.run()
    check(
        "existing launcher symlink is updated without changing its target",
        result.returncode == 0
        and target.resolve() == fixture.project / "bin/orask"
    ...
    for name, override in (
        ("missing", str(scratch / "missing-python")),
        ...
        result = subprocess.run(
            [
                "/bin/bash",
                "-c",
                'source "$1"; orask_find_python "$2"',
                "_",
                str(helper),
                str(fixture.project),
            ],
            env=fixture.env | {"ORASK_PYTHON": override},
            check=False,
            text=True,
            capture_output=True,
            timeout=10,
    ...
    result = fixture.run({"ORASK_PYTHON": str(scratch / "missing-python"), "ORASK_BOOTSTRAP": "1"})
    check(
        "invalid explicit installer interpreter refuses before writes",
        result.returncode != 0
    ...
    check(
        "relative explicit interpreter is refused before writes",
        result.returncode != 0
        and not (fixture.project / ".orask-python").exists()
        and not (fixture.home / ".local/bin").exists(),
    )
    ...
    for kind in ("symlink", "hardlink"):
        ...
        result = fixture.run()
        check(
            f"interpreter pin {kind} cannot overwrite another file",
    ...
    check(
        "interpreter pin replacement is private and backs up the old bytes",
        result.returncode == 0
        and pin.read_text() == sys.executable + "\n"
        and pin.stat().st_mode & 0o777 == 0o600
        and len(backups) == 1
    ...
    result = fixture.run()
    check(
        "unchanged interpreter pin does not create extra backups",
        result.returncode == 0 and len(list(fixture.project.glob(".orask-python.*.bak"))) == 1,
    )
    ...
    check(
        "pin backup failure preserves pin and stops before registrations",
        result.returncode != 0
        and pin.read_text() == "/previous/python\n"
        and not fixture.calls()
        and not (fixture.home / ".local/bin").exists(),
    )
    ...
    result = fixture.run()
    check(
        "oversized config reports the installer limit without replacement",
        result.returncode != 0
        and "32 MiB" in result.stderr
    ...
    for launcher in ("orask", "openrouter-mcp"):
        ...
        check(
            f"{launcher} invalid interpreter has no stdout",
            result.returncode != 0 and not result.stdout and "ORASK_PYTHON" in result.stderr,
        )
...
""")
    result = subprocess.run(
        [str(fixture.project / "bin/orask"), "--help"],
        env=fixture.env,
        check=False,
        capture_output=True,
        text=True,
    ...
    check(
        "CLI launcher works without the MCP dependency",
        result.returncode == 0 and "usage:" in result.stdout and not result.stderr,
    )
    ...
    for validation in (parser, None):
        ...
        for index, original in enumerate(fixtures):
            ...
            if parser is not None:
                ...
                expected["openrouter"] = parser.loads(
                    install_config.codex_block("/tmp/server", sys.executable)
                )["mcp_servers"]["openrouter"]
                preserved = parser.loads(updated) == before
            check(
                f"scanner preserves semantics {index} (tomllib={validation is not None})",
                preserved
                and install_config.update_codex_text(updated, "/tmp/server", sys.executable)
                == updated,
            )
        for original in (
            ...
            try:
                ...
                refused = True
            check(
                f"scanner refuses ambiguous/invalid form (tomllib={validation is not None})",
                refused,
            )
    ...
    with mock.patch.object(fcntl, "flock", side_effect=OSError("injected lock failure")):
        try:
            ...
            refused = True
    check(
        "Codex lock failure refuses any config write",
        refused and locked_config.read_text() == prefix,
    )
    ...
    with held_lock.open("a+") as handle:
        ...
        try:
            ...
            refused = True
    check(
        "concurrent installer lock refuses immediately",
        refused and locked_config.read_text() == prefix,
    )
    ...
    try:
        ...
        refused = True
    check(
        "existing backup cannot be replaced",
        refused
        and existing_backup.read_text() == "keep backup"
    ...
    for grew in (False, True):
        ...
        with (
            mock.patch.object(install_config, "MAX_CONFIG_BYTES", 8, create=True),
