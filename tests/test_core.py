...
FAKE = [
    ...
    },
    {
        ...
        "pricing": {"prompt": "0.00000045", "completion": "0.00000225"},
...
check(
    "a supported effort is left alone", core.clamp_effort("z-ai/glm-5.3", "high") == ("high", None)
)
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    check("relative paths resolve against cwd", f"## File: {root.resolve() / 'a.py'}" in body)
    check("the same file named twice is only sent once", body.count("print('hello')") == 1)
...
check(
    "thread history is replayed in order",
...
check(
    ...
    abs(
        core.actual_cost("z-ai/glm-5.3", {"prompt_tokens": 1000, "completion_tokens": 1000})
        - (0.0000014 + 0.0000044) * 1000
    )
    < 1e-9,
)
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    check(
        "refused file content never reaches the prompt",
    ...
    check(
        "an ordinary source file still goes through",
    ...
    check(
        "the denylist does not catch innocent names like tokenizer.py",
    ...
    check("allow_secret_files overrides the denylist", "SECRET=abc123" in messages[-1]["content"])
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    check("an uppercase secret filename is still refused", any("REFUSED" in n for n in notes))
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    check(
        "the same name is stable across calls",
    ...
    with _cf.ThreadPoolExecutor(max_workers=6) as pool:
        list(pool.map(lambda i: core.save_thread("shared", f"q{i}", f"a{i}", "m"), range(6)))
    kept = core.load_thread("shared")
    check(
        "concurrent writes to one thread lose nothing",
        len(kept) == 12,
        f"{len(kept)} of 12 messages kept",
    )
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    check(
        "symlinked secret content never reaches the prompt",
        "PRIVATE KEY MATERIAL" not in messages[-1]["content"],
    )
    ...
    check(
        "a symlink to an ordinary file still works", "x = 1" in messages[-1]["content"], str(notes)
    )
    ...
    check(
        "a custom deny pattern does not disable the built-ins", any("REFUSED" in n for n in notes)
    )
    core._config_cache = dict(
        cfg,
        deny_file_patterns=["*/nothing/*"],
        deny_file_patterns_replace=True,
    )
    _, notes = core.build_messages("q", files=[str(secret)])
    check("replace mode is available but must be explicit", not any("REFUSED" in n for n in notes))
    core._config_cache = cfg
...
check(
    "lowering max_tokens is reported", any("lowered max_tokens" in n for n in _notes), str(_notes)
)
...
_fit, _win, _notes = core.fit_context("mistralai/mistral-large-2512", 4000000, 32000, compress=True)
check(
    "context_compression turns that refusal into a capped answer",
    _fit <= _win // 2
    and _fit >= core.MIN_ANSWER_TOKENS
...
check(
    ...
    core._tristate(None) is None
    and core._tristate("yes") is None
    and core._tristate(False) is False
    and core._tristate(True) is True,
)
...
try:
    ...
    check(
        "block policy refuses a model with no catalogue pricing",
        "cannot be checked" in str(exc),
        str(exc)[:80],
    )
core._config_cache = cfg
...
try:
    ...
    check(
        "an open socket on stdin does not hang the CLI",
        proc.returncode == 0,
...
try:
    ...
    check(
        "a socket that never closes still returns",
        proc.returncode == 0,
...
check(
    "context with no question is never guessed at",
...
try:
    ...
    check(
        "the refusal names the argument and shows the shape",
...
msgs, notes = core.build_messages(
    None,
    context="<question>Recovered question here</question>",
    files=None,
)
check("a recovered question reaches the prompt", "Recovered question here" in msgs[-1]["content"])
check(
    "the prompt carries the shape warning for the caller",
    any("own argument" in n for n in notes),
    str(notes)[:90],
)
...
check(
    "a bare string in files is treated as one path",
    sum(1 for n in notes if "not found" in n) == 1,
    str(notes)[:100],
)
...
_cat_cfg = dict(
    ...
    categories={
        "coding": {
            ...
            "aka": ["code", "programming"],
            "why": "x",
            "measured": "2026-09-10",
        },
        "long_context": {
            "models": ["z-ai/glm-5.3"],
            "aka": ["long context", "whole codebase"],
            "why": "x",
            "measured": "2026-09-10",
        },
        "retired": {
            "models": ["moonshotai/kimi-k9-retired"],
            "aka": [],
            "why": "x",
            "measured": "x",
        },
        "banned": {
            "models": ["openai/gpt-6-astra", "z-ai/glm-5.3"],
            "aka": [],
            "why": "x",
            "measured": "x",
        },
...
check(
    "a category name is case and space insensitive",
...
check(
    "a phrase lifted from the user resolves",
...
check(
    "the longest matching label wins, not the first",
    core.resolve_category("long context")[0] == "long_context",
)
...
check(
    "a category returns its models in order",
    slugs == ["moonshotai/kimi-k3", "z-ai/glm-5.3"],
    str(slugs),
)
...
check(
    "a retired pin self-heals to the closest live model",
    slugs == ["moonshotai/kimi-k3"],
    str(slugs),
)
...
check("dropping an excluded vendor is reported", any("excluded" in n for n in notes), str(notes))
...
try:
    ...
    check(
        "that error names the category and the config file",
        "gone" in str(exc) and "models.json" in str(exc),
        str(exc)[:80],
    )
...
try:
    ...
    check(
        "the refusal lists the real categories",
        "coding" in str(exc) and "list_llm_categories" in str(exc),
        str(exc)[:90],
    )

check(
    "every configured category is listed",
...
check(
    "every shipped category names two models",
    all(len(core.as_list(c.get("models"))) == 2 for c in _cats.values()),
    str(
        {
            k: len(core.as_list(v.get("models")))
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    check(
        "a png is classified as an image",
        core.classify_attachment(root / "shot.png") == ("image", "image/png"),
    )
    check(
        "a pdf is classified as a pdf",
        core.classify_attachment(root / "spec.pdf") == ("pdf", "application/pdf"),
    )
    check(
        "a wav is classified as audio",
        core.classify_attachment(root / "clip.wav") == ("audio", "wav"),
    )
    check("source code is not an attachment", core.classify_attachment(root / "app.py") is None)
    check(
        "magic bytes beat a missing extension",
    ...
    check(
        "attachments turn the user message into content parts",
    ...
    check(
        "the image is a base64 data url",
    ...
    check(
        "the pdf is a base64 data url",
        file_part["file"]["file_data"].startswith("data:application/pdf;base64,"),
    )
    check(
        "source alongside an attachment is still inlined as text",
        "print('hi')" in content[0]["text"],
    )
    check(
        "the text part lists what was attached",
        "## Attached files" in content[0]["text"] and "shot.png" in content[0]["text"],
    )

    summary = core.attachment_summary(messages)
    check(
        "the attachment summary counts each kind",
        summary["image"] == 1 and summary["pdf"] == 1 and summary["total"] == 2,
        str(summary),
    )
    check(
        "text_chars ignores the base64 payload",
        core.text_chars(messages) < 2000,
        str(core.text_chars(messages)),
    )

    # kimi's fake catalogue entry takes text and image but not audio
    _, notes = core.build_messages(
        "transcribe",
        files=[str(root / "clip.wav")],
        model_slug="moonshotai/kimi-k3",
    )
    check(
        "audio is refused for a model that cannot take it",
        any("does not accept" in n for n in notes),
        str(notes)[:100],
    )
    ...
    check(
        "audio sends bare base64 and a format, not a data uri",
        audio
        and audio[0]["input_audio"]["format"] == "wav"
    ...
    check(
        "an attachment is not counted against max_input_chars",
    ...
    check(
        "an oversized attachment is skipped with a note",
        any("read ceiling" in n for n in notes),
        str(notes)[:100],
    )
    core._config_cache = dict(cfg, max_attachments=1)
    messages, notes = core.build_messages(
        "q",
        files=[str(root / "shot.png"), str(root / "spec.pdf")],
    )
    check(
        "the attachment count ceiling holds",
        any("attachment limit" in n for n in notes),
        str(notes)[:100],
    )
    core._config_cache = cfg
    ...
    check(
        "the denylist applies to attachments too",
        any("REFUSED" in n for n in notes),
        str(notes)[:100],
    )
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    check(
        "max_dir_files bounds an expansion",
...
core.save_thread(
    "carry", "what is in it?", "a codeword", "m", annotations=_ANNOT, attachments=[_FILE_PART]
)
_hist = core.load_thread("carry")
check(
    "a carried attachment rides on the user turn",
    isinstance(_hist[0]["content"], list)
    and any(p.get("type") == "file" for p in _hist[0]["content"]),
)
check(
    "the question text is still the first part of that turn",
...
check(
    "the replayed turn still carries the file part",
    any(
        p.get("type") == "file"
        for m in _user
...
check(
    "the replayed assistant turn carries the annotations",
    any(m.get("annotations") == _ANNOT for m in _msgs if m["role"] == "assistant"),
)
check(
    "carrying a document forward is reported",
    any("carried 1 earlier attachment" in n for n in _notes),
    str(_notes)[:110],
)
...
check(
    "a turn with no attachment stays a plain string",
...
check(
    "attachment_summary counts replayed parts, not just new ones",
    core.attachment_summary(_msgs)["pdf"] == 1,
)
check("sent_attachments reports only what this call added", core.sent_attachments(_msgs) == [])
...
check(
    "an image_too_large error explains itself",
    "scale it down" in core._http_message(400, _img_err),
    core._http_message(400, _img_err)[:100],
)
_unknown = json.dumps(
    {"error": {"code": 400, "message": "nope", "metadata": {"error_type": "something_new"}}}
)
check(
    "an unrecognised error_type is still surfaced",
    "something_new" in core._http_message(400, _unknown),
)
check(
    "a plain error still reports its status and message",
...
try:
    ...
    check(
        "a missing output cap cannot price the answer at zero",
        "over the" in str(exc),
        str(exc)[:80],
    )
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    try:
        ...
        check(
            "a bad thread_max_messages cannot lose a paid answer",
            wrote is True,
            f"save_thread returned {wrote}",
        )
        check(
            "and the answer is actually there",
            any(m.get("content") == "an answer already paid for" for m in core.load_thread("paid")),
        )
    except Exception as exc:
...
try:
    ...
    check(
        "an error inside a 200 body is raised, not read as an answer",
        "429" in str(exc) and "rate limited" in str(exc),
        str(exc)[:90],
    )
    check(
        "and it carries the typed hint rather than a raw dump",
        "rate_limited" in str(exc) or "rate limited by OpenRouter" in str(exc),
        str(exc)[:90],
    )
...
check(
    "bridge spend counts a billed non-answer",
    abs(_usage["bridge_spend_usd"] - 0.03) < 1e-9,
    str(_usage["bridge_spend_usd"]),
)
check(
    "and reports how many calls were billed",
    _usage["bridge_calls_billed"] == 2,
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    check(
        "a question recovered from context is stored, not a null turn",
        [m["role"] for m in _kept] == ["user", "assistant"],
        str([m.get("role") for m in _kept]),
    )
    check(
        "and the stored question is the recovered text",
        _kept[0]["content"] == "Is the plan sound?",
        repr(_kept[0]["content"]),
    )
    ...
    core.ask(
        None,
        model="kimi",
        thread="recovered-file",
        files=[str(_png)],
        context="<question>What is in this image?</question>",
    )
    _first = core.load_thread("recovered-file")[0]["content"]
    check(
        "an attached turn carries real question text, never null",
        isinstance(_first, list)
        and _first[0] == {"type": "text", "text": "What is in this image?"},
        str(_first[0])[:80],
    )
    ...
    check(
        "a null text part is dropped but the file part survives",
        _healed[1]["content"] == [_FILE_PART],
        str(_healed[1]["content"])[:80],
    )
    core._request = _no_network
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    check(
        "a csv whose first column is ID3 is not an mp3",
...
check(
    "the role fallback note names the role actually used",
    any("used 'reviewer'" in n for n in _notes),
    str(_notes)[:90],
)
...
check(
    "and says why the duplicate was dropped",
...
check(
    "and is reported there rather than killing the panel",
...
check(
    "a catalogue outage is attempted once per cooldown, not once per lookup",
    _attempts["n"] == 1,
    f"{_attempts['n']} network attempts for 4 lookups",
)
check(
    "and the later refusals say the retry was skipped on purpose",
    len(_refusals) == 4 and "not retried" in _refusals[-1],
    _refusals[-1][:80],
)
core._request = _no_network
core._catalog_failed_at = 0.0
...
core.get_catalog = lambda refresh=False, allow_stale=True: FAKE
check(
    "the catalogue index follows a replaced cache rather than going stale",
...
try:
    ...
    check(
        "a pipe that never closes refuses an incomplete prompt",
        "ORASK_STDIN_DEADLINE" in str(exc) and _elapsed < 5,
        str(exc)[:100],
    )
os.close(_read_fd)
os.close(_write_fd)
...
for _key in ("mcp_allow_secret_files", "mcp_allow_expensive"):
    ...
    check(
        f"and {_key} says how to permit it", bool(_why) and _key in (_why or ""), (_why or "")[:70]
    )
    ...
    core._config_cache = cfg
    check(
        f"{_key} is silent when nothing was requested",
...
for _path, _label in (
    ...
    check(
        f"{_label} is on the denylist",
        bool(core.denied_by_policy(Path(_path), core.DEFAULT_DENY_PATTERNS)),
        _path,
    )

for _path in ("/proc/cpuinfo", "/proc/meminfo", "/home/u/project/config.py"):
    check(
        f"{_path} is still sendable",
...
_big_pdf = {
    "type": "file",
    "file": {"file_data": "data:application/pdf;base64," + "A" * 8_000_000, "filename": "scan.pdf"},
}
_summary = core.summarize_parts([_big_pdf])
check(
    "the attachment summary reports pdf bytes",
    _summary["pdf_bytes"] > 5_000_000,
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    core._config_cache = dict(cfg, max_cost_usd_per_call=1.0, mistral_ocr_usd_per_1k_pages=200.0)
    try:
        core.ask("q", model="kimi", files=[str(_scan)], pdf_engine="mistral-ocr", max_tokens=100)
        check("an ocr page charge is counted by the cost guard", False, "guard did not fire")
    except core.OpenRouterError as exc:
        check(
            "an ocr page charge is counted by the cost guard", "over the" in str(exc), str(exc)[:80]
        )
        check(
            "and the refusal says the page charge is why",
            "mistral-ocr page charges" in str(exc),
            str(exc)[:110],
        )
    except AssertionError:
        check(
            "an ocr page charge is counted by the cost guard",
            False,
    ...
    core._request = lambda method, path, payload=None, timeout=60.0, retries=3: _answered
    _res = core.ask(
        "q", model="kimi", files=[str(_scan)], pdf_engine="cloudflare-ai", max_tokens=100
    )
    check(
        "the free engine adds no page charge",
        not any("bills per page" in n for n in _res["notes"]),
        str(_res["notes"])[:90],
    )
...
check(
    "every default_panel entry is a configured alias or a full slug",
...
check(
    "every alias points at a full slug, not another alias",
    all("/" in v for v in _aliases.values()),
    str(_aliases),
)
check(
    "the packaged aliases are the four in service",
    set(_aliases) == {"kimi", "glm", "grok", "gemini"},
    str(sorted(_aliases)),
)
...
check(
    "an alias may name a vendor the categories exclude",
    any(v.startswith(_excluded) for v in _aliases.values()),
    f"aliases {sorted(_aliases.values())} vs excluded {_excluded}",
)
check(
    "and no category pins a model from an excluded vendor",
    not [m for row in core.list_categories() for m in row["models"] if m.startswith(_excluded)],
    str([m for row in core.list_categories() for m in row["models"] if m.startswith(_excluded)]),
)
...
for _phrase, _want in [
    ...
    check(
        f"'{_phrase}' resolves to {_want}",
        _got is not None and _got[0] == _want,
        str(_got[0] if _got else None),
    )
check(
    "a phrase matching no category is refused, not guessed",
    core.resolve_category("use the underwater basket weaving llms") is None,
)
check(
    "every category names the evidence behind its pick",
...
check(
    "the installer derives the Codex tool list instead of repeating it",
    "-m orask.install_config" in _install_src
    and "from .core import MCP_TOOLS" in _install_config_src,
)
check(
    "the live MCP protocol test expects the same set",
    sorted(
        re.findall(
            r'"([a-z_]+)"',
            (ROOT / "tests" / "test_mcp_stdio.py")
            .read_text()
            .split("EXPECTED_TOOLS = {")[1]
            .split("}")[0],
        )
    )
    == sorted(core.MCP_TOOLS),
)


# ---- guides are local files, and the topic name is not to be trusted ------
_guides = core.list_guides()
check("the packaged guides are found", len(_guides) >= 5, ", ".join(g["topic"] for g in _guides))
check(
    "every guide declares when to read it and a parseable verified date",
...
check(
    "an outline lists headings without returning the body",
    len(_outline["sections"]) > 3,
    f"{len(_outline['sections'])} sections",
)
check(
    "every outline entry carries its own length, for budgeting a read",
    all(s["lines"] > 0 for s in _outline["sections"]),
)
check(
    "the line count is the body, not the front matter",
...
for _bad in ("../../../etc/passwd", "/etc/passwd", "../README", "..", "a/b", "%2e%2e/x"):
    check(
        f"a guide path cannot escape the guides directory ({_bad})", core._guide_path(_bad) is None
    )
...
check(
    "and its contents cannot be reached through search",
...
check(
    "a configured directory wins over the packaged copy of the same topic",
...
try:
    ...
    check(
        "an unknown guide is refused with the list of real ones",
        "Available guides" in str(_exc),
        str(_exc)[:80],
    )
try:
    ...
    check("an unknown section names the real sections", "Sections:" in str(_exc), str(_exc)[:80])
...
check(
    ...
    _heads == ["Real one", "Real two"],
    str(_heads),
)
_starts = core._guide_headings(_body)
check(
    "and a fenced heading does not cut a section short",
...
check(
    "a horizontal rule is not mistaken for front matter",
    _meta == {} and "Intro para" in _rule_body,
    str(_meta),
)
_meta2, _ = core._guide_split("\ufeff---\ntriggers: t\nverified: 2026-01-01\n---\n\nbody\n")
check(
    "a byte order mark does not hide the front matter",
    _meta2.get("verified") == "2026-01-01",
    str(_meta2),
)

_found = core.search_guides("pipefail")
check(
    "search finds a rule and names the section holding it",
    any(h["topic"] == "bash" and h["section"] for h in _found["hits"]),
    str(_found["hits"][:1])[:100],
)
check(
    "search matches heading text, not only body lines", core.search_guides("Heredocs")["total"] > 0
)
_small = core.search_guides("the", limit=3)
check(
    "a truncated search says so and still counts every match",
    _small["truncated"] and _small["total"] > len(_small["hits"]),
    f"{len(_small['hits'])} of {_small['total']}",
)
check(
    "a truncated search is not filled from one guide alone",
...
check(
    "front matter is stripped from the returned text",
    not _whole.lstrip().startswith("---"),
    _whole[:40],
)
check(
    "the packaged guides directory is always searched",
    core.GUIDES_DIR.resolve() in core.guide_dirs(),
)
check(
    "a guide_dirs path is not comma-split the way files are", core.guide_dirs()[0] != Path("/nope")
)
...
check(
    "a file with no front matter is indexed by its title",
    _plain["triggers"] == "Redis How-To and Best Practices",
    _plain["triggers"],
)
...
for _key in ("mcp_allow_secret_files", "mcp_allow_expensive"):
    for _value in ("false", "true", 1, [True]):
        ...
        check(
            f"{_key} requires JSON true ({_value!r})", core.override_allowed(_key, True)[0] is False
        )
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    check(
        "denied original symlink name stays denied after resolving",
    ...
    check(
        "relocated API key file is always denied",
...
core.get_catalog = lambda refresh=False, allow_stale=True, entry=_entry: [entry]
check(
    "per-request price contributes to the cost guard",
    core.estimate_call_cost(_entry["id"], 100, 100) == (2.0, True),
)
core.get_catalog = lambda refresh=False, allow_stale=True: FAKE
check(
    "reported zero cost is authoritative",
...
for _content, _reasoning, _finish in (
    ("", "unfinished analysis", "length"),
    ("partial findings", "analysis", "length"),
    ("", "reasoning only", "stop"),
):
    _response = {
        "choices": [
            {"message": {"content": _content, "reasoning": _reasoning}, "finish_reason": _finish}
        ],
        "usage": {
            "prompt_tokens": 100,
    ...
    check(
        "unfinished reply retains usage and diagnostic recovery",
        _result["usage"]["cost_usd"] == 0.02 and any("max_tokens" in n for n in _result["notes"]),
    )
...
check(
    "model discovery exposes bridge budget defaults and cost guard",
    _info.get("bridge_limits", {}).get("default_max_tokens") == cfg["default_max_tokens"]
    and _info.get("bridge_limits", {}).get("max_cost_usd_per_call") == cfg["max_cost_usd_per_call"],
)


# ---- MCP effort is enforced, not entrusted to caller prose -----------------
for _effort, _reason in (
    ("low", None),
    ("minimal", None),
    ("none", None),
    ("off", None),
...
try:
    ...
    check(
        "MCP defaults to max regardless of lower configured defaults",
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    check(
        "listed thread names resume the same conversation",
    ...
    check(
        "failed lock cannot cause an unlocked write", wrote is False and path.read_bytes() == before
    )
    for blob in ([], {"messages": 3}, {"messages": [None, {}, {"content": "x"}]}):
        path.write_text(json.dumps(blob))
        try:
            check(
                f"corrupt thread shape is safely handled ({blob})",
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    try:
        ...
        os.umask(old_umask)
    check(
        "call logs are private regardless of umask", core.CALL_LOG.stat().st_mode & 0o777 == 0o600
    )
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    check(
        "reads refuse symlink ancestors after path selection",
